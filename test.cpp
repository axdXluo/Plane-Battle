...
const int Enemy_Num1 = 10;   //敌方飞机1的数量
...
struct Plane myPlane = { Init_Plane_x_label, Init_Plane_y_label, Plane_speed, Plane_HP, 102, 126, 0, true };
...
void Init_Enemy()
{
	for (int i = 0; i < Enemy_Num1; i++)
		EnemyPlane1[i] = {0, 0, Enemy_Speed1, Enemy_HP1, 57, 43, 1, false };
	for (int i = 0; i < Enemy_Num2; i++)
		EnemyPlane2[i] = { 0, 0, Enemy_Speed2, Enemy_HP2, 69, 99, 2, false };
	for (int i = 0; i < Enemy_Num3; i++)
		EnemyPlane3[i] = { 0, 0, Enemy_Speed3, Enemy_HP3, 165, 261, 3, false };
}

//生成敌方飞机1
void Creat_Enemy1()
{
	cout << "Enemy_Recent_Num1:" << Enemy_Recent_Num1 << endl;
	//生成类型1敌机
...
void Key_Command()
{
	...
	{
		if (Timer(Enemy_interval_ms3, 4))	//敌方机型1当前数量为0后，开始创建敌方机型2
			Creat_Enemy3();
	}
